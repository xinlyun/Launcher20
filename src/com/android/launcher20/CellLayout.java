...
import com.android.launcher20.FolderIcon.FolderRingAnimator;
...
public class CellLayout extends ViewGroup {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // We draw the pressed or focused BubbleTextView's background in CellLayout because it
        // requires an expanded clip rect (due to the glow's blur radius)
        boolean disablePressedIconBackground = getResources().getBoolean(R.bool.config_DisablePressedIconBackground);
        if ((mPressedOrFocusedIcon != null) && !disablePressedIconBackground) {
    ...
    public void setTagToCellInfoForPoint(int touchX, int touchY) {
        Log.d("CellLayout","setTagToCellInfoForPoint"+"x:"+touchX+"  Y:"+touchY);
...
//        Log.d("CellLayout","onInterceptTouchEvent   "  +CellLayout.this.toString()+"  x:"+ev.getRawX()+"  y:"+ev.getRawY());
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            setTagToCellInfoForPoint((int) ev.getRawX(), (int) ev.getRawY());
        }
...
//        Log.d(TAG,"getDirectionVectorForDrop"+" x:"+spanX+" y:"+spanY);
...
//        Log.d(TAG,"getViewsIntersectingRegion  "+" size:"+intersectingViews.size());
        if(intersectingViews.size()>=1){
            for (View view:intersectingViews) {
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    private OnAppWidgetHostViewPosiChangeListener appWidgetHostViewPosiChangeListener;
    public void setOnAppWidgetHostViewPosiChangeListener(OnAppWidgetHostViewPosiChangeListener appWidgetHostViewPosiChangeListener){
        this.appWidgetHostViewPosiChangeListener = appWidgetHostViewPosiChangeListener;
    }
}
interface OnAppWidgetHostViewPosiChangeListener{
